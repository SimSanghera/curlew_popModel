...
PVA_NI_10 <- as.data.frame(cbind(
  PVA_min,
  PVA_med,
  PVA_max
))

PVA_NI_15 <- as.data.frame(cbind(
  PVA_min,
  PVA_med,
  PVA_max
))

PVA_NI_20 <- as.data.frame(cbind(
  PVA_min,
  PVA_med,
  PVA_max
))

PVA_NI_25 <- as.data.frame(cbind(
  PVA_min,
  PVA_med,
  PVA_max
))

PVA_NI_30 <- as.data.frame(cbind(
  PVA_min,
  PVA_med,
  PVA_max
))

PVA_NI_35 <- as.data.frame(cbind(
  PVA_min,
  PVA_med,
  PVA_max
))
...
PVA_NI_10_means <- rowMeans(PVA_NI_10)
...
PVA_NI_A3_max <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
NI_A_means_max <- as.tibble(cbind(
  rowMeans(PVA_NI_A1[, -1])/2,
  rowMeans(PVA_NI_A2[, -1])/2,
  rowMeans(PVA_NI_A3_max[, -1])/2,
  rowMeans(PVA_NI_A4[, -1])/2
))

NI_A_means_max$years <- years
names(NI_A_means_max) <- c("A1", "A2", "A3", "A4", "years")
...
NI_A <- gather(NI_A_means_max,
                   scenario, pop_size, gathercols)


write.table(NI_A_means_max,
            "data/curlew_HS_NI_A_means_max_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(NI_A,
            "data/HS_NI_A_max_simple_1.csv",
            sep = ",",
            row.names = FALSE)
