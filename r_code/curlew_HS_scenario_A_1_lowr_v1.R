...
write.table(simul.in,
            "data/NI_HS_scenario_A_1base_mplr_parameters.csv",
            sep = ",",
            row.names = FALSE)
...
# Write to save file
saveRDS(sim.out,
        "data/NI_base_mod_sims_mplr_1.RDS")
...
names(sim.out) <- names.NI.sim.out
NI.sim.out <- sim.out
...
write.table(NI.sim.out.df.long,
            "data/NI_HS_base_A1_means_fullsims_1.csv",
            sep = ",",
            row.names = FALSE)
...
bettermatch.225 <- which(NI.sim.out.225[31 ,] < 200,
                     arr.ind = TRUE)
...
write.table(NI.sim.out.df.match,
            "data/NI_HS_base_A1_lowPlowR_means_match20_1.csv",
            sep = ",",
            row.names = FALSE)
