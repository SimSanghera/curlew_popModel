...
site.pop      <- 37                     # number of breed pairs on site
# glen  = 57
# lle = 37
# low = 4
# lowermed = 8
...
return.p      <- 1.0                   # probability of breeding birds 
...
write.table(simul.in.simple,
            "data/curlew_HS_lle_simple_params_10c.csv",
            sep = ",",
            row.names = FALSE)
...
PVA_glen_base <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_glen_10 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_glen_15 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_glen_20 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_glen_25 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_glen_30 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_glen_35 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_base <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_10 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_15 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_20 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_25 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_30 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_35 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_40 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
glen20bp_means <- as.tibble(cbind(
  rowMeans(PVA_glen[, -1])/2,
  rowMeans(PVA_glen_rem[, -1])/2,
  rowMeans(PVA_glen_10[, -1])/2,
  rowMeans(PVA_glen_15[, -1])/2,
  rowMeans(PVA_glen_20[, -1])/2,
  rowMeans(PVA_glen_25[, -1])/2,
  rowMeans(PVA_glen_30[, -1])/2,
  rowMeans(PVA_glen_35[, -1])/2
))

glen20bp_means_2 <- as.tibble(cbind(
  PVA_glen_base_means,
  glen20bp_means
))

glen20bp_means$years <- years
names(glen20bp_means) <- c("base", "rem", "10", "15", "20", "25", "30", "35", "years")



lle20bp_pva <- as.tibble(rbind(
  PVA_lle,
  PVA_lle_10,
  PVA_lle_15,
  PVA_lle_20,
...
lle20bp_means <- as.tibble(cbind(
  rowMeans(PVA_lle[, -1])/2,
  rowMeans(PVA_lle_rem[, -1])/2,
  rowMeans(PVA_lle_10[, -1])/2,
  rowMeans(PVA_lle_15[, -1])/2,
  rowMeans(PVA_lle_20[, -1])/2,
  rowMeans(PVA_lle_25[, -1])/2,
  rowMeans(PVA_lle_30[, -1])/2,
  rowMeans(PVA_lle_35[, -1])/2
))



lle20bp_means$years <- years
names(lle20bp_means) <- c("base", "rem", "10", "15", "20", "25", "30", "35", "years")

num_clutches <- "num_clutches"
pop_size <- "population size (breeding pairs)"
gathercols <- c("base", "rem", "10", "15", "20", "25", "30", "35")


glen20bp <- gather(glen20bp_means,
                          num_clutches, pop_size, gathercols)

lle20bp <- gather(lle20bp_means,
                   num_clutches, pop_size, gathercols)




write.table(glen20bp,
            "data/curlew_glen_20bp_means_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(lle20bp,
            "data/curlew_lle_20bp_means_simple_1.csv",
            sep = ",",
            row.names = FALSE)

darPal <- wes_palette("Darjeeling2")



glen_plot <- ggplot(glen20bp) +
  ...
  geom_smooth(aes(x = years, y = pop_size,
                  ...
                  colour = num_clutches),
              size = 1) +
  scale_colour_manual(values = c(foxPal, darPal)) +
  xlab("Years") +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)), 
                     limits = c(0, 110),
                     breaks = seq(0, 110, 10)) +
  labs(colour = "Scenario & Number of Clutches") +
