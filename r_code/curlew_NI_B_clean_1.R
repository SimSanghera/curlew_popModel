...
# Underlying productivity increases due to improved conservation management
#   over time
#   1. Scenario B1: no head-starting - this is the BASELINE MODEL
#       - 0 clutches added
#   2. Scenario B2: enough head-starting to prevent decline (i.e. minimum 
...
#         assumption
#   3. Scenario B3: maximum head-starting that is reasonably possible
#       (e.g. collecting 20 clutches), duration 5 years, HS birds
#       survive & return in line with wild-reared curlew
#       For NI this was 60
#   4. Scenario B4: worst case scenario - maximum collection strategy is taken
...
breed.fail      <- 0.2                      # prob breeding season is total 
# failure
...
# Bird matrix tables for subsequent years with increased productivity and 
#   survival
...
write.table(simul.in,
            "data/curlew_NI_A_params_0c.csv",
            sep = ",",
            row.names = FALSE)
...
write.table(simul.in.simple,
            "data/curlew_NI_A_simple_params_0c.csv",
            sep = ",",
            row.names = FALSE)
...
PVA <- function(
  ...
  for (rep in 1:n.reps) {
    ...
    for (y in 2:(n.years + 1)) {
      ...
      # Different years now use separate matrices for increased prod., surv.
      if (y == 1) {
...
system.time(
  ...
  NI.sim.out <- foreach(s = c(1:dim(simul.in.simple)[1]),
...
PVA_NI_base_model <- as.data.frame(
  cbind(
    years,
    min.PVA,
    med.PVA,
...
write.table(PVA_NI_base_model,
            "data/chs_NI_scenario_B_PVA_base.csv",
            sep = ",",
            row.names = FALSE)
