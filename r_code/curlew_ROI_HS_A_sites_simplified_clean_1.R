...
# ROI.pop.size <- 110                         # Sean Kelly 2022
...
write.table(simul.in,       # or simul.in.simple
            "data/curlew_HS_simple_params_10c.csv", # add note for clutches added e.g. 10c
            sep = ",",
            row.names = FALSE)
...
system.time(
  ...
  ROI.sim.out <- foreach(s = c(1:dim(simul.in.simple)[1]),
                         .packages = "popbio") %dopar% {
                           ...
                           # start PVA run 
                           # change simul.in or simul.in.simple
...
min.PVA.corrib <- testPVA   # where s = 1
med.PVA.corrib <- testPVA   # where s = 2
max.PVA.corrib <- testPVA   # where s = 3

# create vector of years
years <- seq(2023, 2053, 1)

PVA_corrib_base_model <- as.data.frame(
  cbind(
    years,
    min.PVA.corrib,
    med.PVA.corrib,
...
# Write to file
# Change site name in file and model
write.table(PVA_corrib_base_model,
            "data/chs_corrib_PVA_base.csv",
            sep = ",",
            row.names = FALSE)


# This needs to be done for all sites, with different numbers of clutches
#   added (0, 10, 15, 20, 25, 30, 35, 40)
