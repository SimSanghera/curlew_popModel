...
## Edit: 16/03/22 change to PVA function to better distinguish pop matrix in 
...
intro.years   <- 0           # years in which hs birds will be
# introduced

phil.p        <- 0.7                    # probability of first years return
# to release site
...
extinct.cutoff  <- c(4, 32, 64, 80)      # below this number, pop is extinct
...
write.table(simul.in,
            "data/ROI_HS_scenario_A1_base_hphr_parameters_2.csv",
            sep = ",",
            row.names = FALSE)
...
# Write to save file
saveRDS(ROI.sim.out,
        "data/ROI_HS_A1_sims_hphr_3.RDS")

ROI.sim.out <- readRDS("data/ROI_HS_base_A1_N100_sims_lphr_2.RDS")
...
write.table(ROI.sim.out.df.long,
            "data/ROI_HS_A1_means_hphr_fullsims_2.csv",
            sep = ",",
            row.names = FALSE)
...
# Lower than x - 1 clutch, 5 clutches, 10 clutches, 20 clutches
# 4, 20, 40, 80
bettermatch.100 <- which(ROI.sim.out.100[31 ,] < 1,
                         arr.ind = TRUE)
...
bettermatch.110 <- which(ROI.sim.out.110[31 ,] < 1,
                         arr.ind = TRUE)
...
bettermatch.120 <- which(ROI.sim.out.120[31 ,] < 1,
                         arr.ind = TRUE)
...
ROI.sim.out.df.match <- data.frame(years = rep(years, times = 3),
                                  initN = rep(ROI.pop.size, each = 31),
                                  mean = c(ROI.sim.out.100.match.mean,
                                           ROI.sim.out.110.match.mean,
                                           ROI.sim.out.120.match.mean),
...
write.table(ROI.sim.out.df.match,
            "data/ROI_HS_A1_hphr_means_match1_1.csv",
            sep = ",",
            row.names = FALSE)
...
system.time(
  ...
  NI.sim.out.extprob <- foreach(s = c(1:dim(simul.in)[1]),
                                .packages = "popbio") %dopar% {
...
for (s in 1:length(NI.sim.out)) {
  ...
  for (t in extinct.cutoff) {
    ...
    out$ext.thresh <- t
    out$outcome <- extinction.bysim(NI.sim.out[s], t)
...
sum(apply(NI.sim.out[[1]], 1,
          function(t) min(t)<100))/ncol(NI.sim.out[[1]])
