...
write.table(simul.in.simple,
            "data/curlew_HS_lle_simple_params_0c.csv",
            sep = ",",
            row.names = FALSE)
...
min.PVA.glen <- testPVA   # where s = 1
med.PVA.glen <- testPVA   # where s = 2
max.PVA.glen <- testPVA   # where s = 3

# create vector of years
years <- seq(2023, 2053, 1)

PVA_glen_base_model <- as.data.frame(
  cbind(
    years,
    min.PVA.glen,
    med.PVA.glen,
...
write.table(PVA_glen_base_model,
            "data/chs_glenwherry_PVA_base.csv",
            sep = ",",
            row.names = FALSE)
