...
hs.clutches   <- 25                     # number of clutches - assume 4 eggs
...
breed.fail    <- 0.3                    # prob the breeding season is a 
...
write.table(simul.in,
            "data/NI_HS_scenario_A3_maxF_hplr_parameters.csv",
            sep = ",",
            row.names = FALSE)
...
write.table(NI.sim.out.df.long,
            "data/NI_HS_A3_means_hplr_fullsims_1.csv",
            sep = ",",
            row.names = FALSE)
...
# Lower than x
bettermatch.200 <- which(NI.sim.out.200[31 ,] < 20,
                         arr.ind = TRUE)
...
bettermatch.225 <- which(NI.sim.out.225[31 ,] < 20,
                         arr.ind = TRUE)
...
bettermatch.250 <- which(NI.sim.out.250[31 ,] < 20,
                         arr.ind = TRUE)
...
bettermatch.275 <- which(NI.sim.out.275[31 ,] < 20,
                         arr.ind = TRUE)
...
bettermatch.300 <- which(NI.sim.out.300[31 ,] < 20,
                         arr.ind = TRUE)
...
write.table(NI.sim.out.df.match,
            "data/NI_HS_A3_hplr_means_match20_1.csv",
            sep = ",",
            row.names = FALSE)
