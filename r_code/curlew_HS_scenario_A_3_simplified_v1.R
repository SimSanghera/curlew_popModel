...
# Population Size
# Population Size
NI.pop.size   <- c(200, 252, 300)      # breeding pairs
NI.pop.size   <- c(100, 150)           # 07/03/22 Change init N to 100 and 150

ROI.pop.size  <- c(100, 150, 200)
...
hs.clutches   <- 10                   # number of clutches - assume 4 eggs
...
a <- c(110, 0.78, 0.278, 0.278, 0.40, 0.78, 0.14, 0.3, 0.3, hs.pop, 5)
b <- c(110, 0.85, 0.326, 0.326, 0.55, 0.85, 0.25, 0.5, 0.6, hs.pop, 5)
c <- c(110, 0.92, 0.378, 0.378, 0.70, 0.70, 0.52, 0.7, 0.9, hs.pop, 5)

simul.in  <- as.data.frame(rbind(a, b, c))
...
write.table(simul.in,
            "data/NI_HS_scenario_A3_N150_maxHS_simplified_parameters_2.csv",
            sep = ",",
            row.names = FALSE)
...
pvaMeans <- rowMeans(HS_PVA)
pvaMeans


#- combine all three PVA models
PVAoutput <- PVaoutput

HS_PVA_low <- HS_PVA
HS_PVA_med <- HS_PVA
HS_PVA_high <- HS_PVA
HS_100_output <- as.data.frame(cbind(years, 
                                     HS_PVA_low, 
                                     HS_PVA_med, 
                                     HS_PVA_high))
...
HS_PVA_med <- HS_PVA
HS_PVA_high <- HS_PVA
HS_10_output <- as.data.frame(cbind(years, 
                                     HS_PVA_low, 
                                     HS_PVA_med, 
                                     HS_PVA_high))

HS_15_output <- as.data.frame(cbind(years, 
                                    HS_PVA_low, 
                                    HS_PVA_med, 
                                    HS_PVA_high))

HS_20_output <- as.data.frame(cbind(years, 
                                    HS_PVA_low, 
                                    HS_PVA_med, 
                                    HS_PVA_high))

HS_25_output <- as.data.frame(cbind(years, 
                                    HS_PVA_low, 
                                    HS_PVA_med, 
                                    HS_PVA_high))

HS_30_output <- as.data.frame(cbind(years, 
                                    HS_PVA_low, 
                                    HS_PVA_med, 
                                    HS_PVA_high))

HS_35_output <- as.data.frame(cbind(years, 
                                    HS_PVA_low, 
                                    HS_PVA_med, 
                                    HS_PVA_high))
...
names(HS_10_output) <- c("years",
                      ...
                      "med values",
                      "high values")
names(HS_15_output) <- c("years",
                         "low values",
                         "med values",
                         "high values")
names(HS_20_output) <- c("years",
                         "low values",
                         "med values",
                         "high values")
names(HS_25_output) <- c("years",
                         "low values",
                         "med values",
                         "high values")
names(HS_30_output) <- c("years",
                         "low values",
                         "med values",
                         "high values")
names(HS_35_output) <- c("years",
                         "low values",
                         "med values",
                         "high values")
...
PVAoutput <- as.data.frame(cbind(years, PVAoutput))
