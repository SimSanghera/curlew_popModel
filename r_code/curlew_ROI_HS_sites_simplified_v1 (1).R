...
site.pop      <- 5                     # number of breed pairs on site
# monaghan = 5
# donegal = 4
# stacks = 1
# ree = 15
...
bird.matrix.2 <- expression(
  ...
  0,          0,     0,                       0,                      0,
  (S.jw*1.05)*phil.p, ((S.jhs*1.05)*capt.fail),  0,  0,                      0,
...
bird.matrix.3 <- expression(
  ...
  0,          0,     0,                       0,                      0,
  (S.jw*1.1)*phil.p, ((S.jhs*1.1)*capt.fail),  0,  0,                      0,
...
bird.matrix.2 <- expression(
  ...
  0,          0,     0,                       0,                      0,
  (S.jw*phil.p)*0, (S.jhs*capt.fail),  0,  0,                      0,
  0,          0,      S.imm*return.p,         0,                      0,
  0,          0,      0,                      S.sub*return.p,     S.ad*return.p
)
...
write.table(simul.in.simple,
            "data/curlew_HS_ROI_simple_params_sites.csv",
            sep = ",",
            row.names = FALSE)
...
PVA <- function(
  ...
  for (rep in 1:n.reps) {
    ...
    for (y in 2:(n.years + 1)) {
      ...
      if (y == 2) {
        pop.size  <- c((F.loop * 0.5 * (init.N * 2))*0,                       #f_w
                       (hs.numbers * (1-capt.fail)),                        #f_hs
                       ((hs.numbers * (1-capt.fail)) * S.jhs.loop * philopatry)*0, #imm
                       0,                                              # sub
                       ((init.N * 2) * S.ad.loop * sitefidelity))      #adu
      } else if (y == 3) {
        pop.size  <- c(projections$stage.vectors[1, y-1]*0,
                       (hs.numbers * (1 - capt.fail)),
                       projections$stage.vectors[3, y-1]*0,
                       projections$stage.vectors[4, y-1]*0,
...
# To obtain means - create array of min, med, max sims
#  then use rowMeans below
#  The number of simulations can vary from 3 (1 each) to 3000 (1000 each).
#  Whilst the larger simulations resulted in a very slow decreasing mean line,
#   the important part is the obtaining of UCL and LCL to show the level of 
#   of uncertainty
PVA_min <- testPVA
PVA_med <- testPVA
PVA_max <- testPVA
...
PVA_stacks_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_stacks <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_stacks_2 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_stacks_means <- as.tibble(cbind(
  rowMeans(PVA_stacks[, -1])/2,
  rowMeans(PVA_stacks_rem[, -1])/2,
  rowMeans(PVA_stacks_2[, -1])/2
))


PVA_mon_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_mon_means <- as.tibble(cbind(
  rowMeans(PVA_mon[, -1])/2,
  rowMeans(PVA_mon_rem[, -1])/2,
  rowMeans(PVA_mon_4[, -1])/2,
  rowMeans(PVA_mon_8[, -1])/2
))

PVA_don_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_don_means <- as.tibble(cbind(
  rowMeans(PVA_don[, -1])/2,
  rowMeans(PVA_don_rem[, -1])/2,
  rowMeans(PVA_don_4[, -1])/2,
  rowMeans(PVA_don_6[, -1])/2
))


PVA_ree_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_ree_10 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_ree_15 <- PVA_ree_10
PVA_ree_15[15:21, 4] <- PVA_ree_15[15:21, 4] + 4


PVA_ree_means <- as.tibble(cbind(
  rowMeans(PVA_ree_rem[, -1])/2,
  rowMeans(PVA_ree[, -1])/2,
  rowMeans(PVA_ree_10[, -1])/2,
  rowMeans(PVA_ree_15[, -1])/2
))


PVA_corrib_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_corrib <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_corrib_10 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_corrib_15 <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_corrib_means <- as.tibble(cbind(
  rowMeans(PVA_corrib[, -1])/2,
  rowMeans(PVA_corrib_rem[, -1])/2,
  rowMeans(PVA_corrib_10[, -1])/2,
  rowMeans(PVA_corrib_15[, -1])/2
))


PVA_mon_means$years <- years
PVA_don_means$years <- years
PVA_stacks_means$years <- years
PVA_ree_means$years <- years
PVA_corrib_means$years <- years

# change names of columns
names(PVA_mon_means) <- c("base", "removed", "HS", "HS_max", "years")
names(PVA_don_means) <- c("base", "removed", "HS", "HS_max", "years")
names(PVA_stacks_means) <- c("base", "removed", "HS", "years")
names(PVA_ree_means) <- c("base", "removed", "HS", "HS_max", "years")
names(PVA_corrib_means) <- c("base", "removed", "HS", "HS_max", "years")
...
gathercols <- c("base", "removed", "HS", "HS_max")

mon <- gather(PVA_mon_means,
                scenario, pop_size, gathercols)

don <- gather(PVA_don_means,
              scenario, pop_size, gathercols)
stacks <- gather(PVA_stacks_means,
              scenario, pop_size, gathercols)
ree <- gather(PVA_ree_means,
              scenario, pop_size, gathercols)
corrib <- gather(PVA_corrib_means,
              scenario, pop_size, gathercols)


write.table(ROI_sites_A,
            "data/HS_ROI_A_means_simple_1.csv",
            sep = ",",
            row.names = FALSE)

corrib_plot <- ggplot(corrib) +
...
# ROI sites
write.table(PVA_mon,
            "data/curlew_HS_PVA_mon_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_mon_rem,
            "data/curlew_HS_PVA_mon_rem_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_mon_4,
            "data/curlew_HS_PVA_mon_4_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_don,
            "data/curlew_HS_PVA_don_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_don_rem,
            "data/curlew_HS_PVA_don_rem_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_don_4,
            "data/curlew_HS_PVA_don_4_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_stacks,
            "data/curlew_HS_PVA_stacks_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_stacks_rem,
            "data/curlew_HS_PVA_stacks_rem_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_stacks_2,
            "data/curlew_HS_PVA_stacks_2_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_ree,
            "data/curlew_HS_PVA_ree_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_ree_rem,
            "data/curlew_HS_PVA_ree_rem_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_ree_10,
            "data/curlew_HS_PVA_ree_10_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_corrib,
            "data/curlew_HS_PVA_corrib_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_corrib_rem,
            "data/curlew_HS_PVA_corrib_rem_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(PVA_corrib_10,
            "data/curlew_HS_PVA_corrib_10_simple_1.csv",
            sep = ",",
            row.names = FALSE)