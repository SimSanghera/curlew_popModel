...
site.pop      <- 110                     # number of breed pairs on site
# glen  = 57
# lle = 37
# low = 4
# lowermed = 8
...
bird.matrix <- expression(
  ...
  0,          0,     0,                       0,                      0,
  S.jw*phil.p, (S.jhs*(1-capt.fail))*phil.p,  0,  0,                      0,
...
bird.matrix.2 <- expression(
  ...
  0,          0,     0,                       0,                      0,
  (S.jw*1.05)*phil.p, ((S.jhs*1.05)*(1-capt.fail))*phil.p,  0,  0,                      0,
...
bird.matrix.3 <- expression(
  ...
  0,          0,     0,                       0,                      0,
  (S.jw*1.1)*phil.p, ((S.jhs*1.1)*(1-capt.fail))*phil.p,  0,  0,                      0,
...
bird.matrix.4 <- expression(
  # f_w       f_hs      imm                     sub                     adult  
  0,          0,    ((prod.rates*1.15)*0.5)*0.005, (prod.rates*0.5)*0.1, (prod.rates*1.2)*0.5,
  0,          0,     0,                       0,                      0,
  (S.jw*1.15)*phil.p, ((S.jhs*1.15)*(1-capt.fail))*phil.p,  0,  0,                      0,
  0,          0,      (S.imm*1.15)*return.p,         0,                      0,
  0,          0,      0,                      S.sub*return.p,     S.ad*return.p
)

bird.matrix.5 <- expression(
  # f_w       f_hs      imm                     sub                     adult  
  0,          0,    ((prod.rates*1.50)*0.5)*0.005, (prod.rates*1.5*0.5)*0.1, (prod.rates*1.5)*0.5,
  0,          0,     0,                       0,                      0,
  (S.jw*1.50)*phil.p, ((S.jhs*1.50)*(1-capt.fail))*phil.p,  0,  0,                      0,
  0,          0,      (S.imm*1.50)*return.p,         0,                      0,
  0,          0,      0,                      S.sub*return.p,     S.ad*return.p
)
...
bird.matrix.2 <- expression(
  ...
  0,          0,     0,                       0,                      0,
  (S.jw*phil.p)*0, (S.jhs*(1-capt.fail))*(phil.p*0),  0,  0,                      0,
  0,          0,      S.imm*return.p,         0,                      0,
  0,          0,      0,                      S.sub*return.p,     S.ad*return.p
)
...
PVA_ROI_B <- as.data.frame(cbind(
  PVA_min,
  PVA_med,
  PVA_max
))

PVA_ROI_B_LCL <- apply(PVA_ROI_B, 1, quantile, probs = 0.1)
PVA_ROI_B_UCL <- apply(PVA_ROI_B, 1, quantile, probs = 0.9)
PVA_ROI_B_means <- rowMeans(PVA_ROI_B)

PVA_ROI_B_df <- as.data.frame(cbind(
  years,
  (PVA_ROI_B_means/2),
  (PVA_ROI_B_LCL/2),
  (PVA_ROI_B_UCL/2)
...
names(PVA_ROI_B_df) <- c("years", "ROI_B1_mean", "ROI_B1_LCL", "ROI_B1_UCL")
write.csv(PVA_ROI_B_df,
          "data/PVA_ROI_B1_means.csv",
          sep = ",",
          row.names = FALSE)
...
PVA_ROI_B2_LCL <- apply(PVA_ROI_B2, 1, quantile, probs = 0.1)
PVA_ROI_B2_UCL <- apply(PVA_ROI_B2, 1, quantile, probs = 0.9)
PVA_ROI_B2_means <- rowMeans(PVA_ROI_B2)

PVA_ROI_B2_df <- as.data.frame(cbind(
  years,
  (PVA_ROI_B2_means/2),
  (PVA_ROI_B2_LCL/2),
  (PVA_ROI_B2_UCL/2)
...
names(PVA_ROI_B2_df) <- c("years", "ROI_B2_mean", "ROI_B2_LCL", "ROI_B2_UCL")
write.csv(PVA_ROI_B2_df,
          "data/PVA_ROI_B2_means.csv",
          sep = ",",
          row.names = FALSE)
...
PVA_ROI_B3_LCL <- apply(PVA_ROI_B3, 1, quantile, probs = 0.1)
PVA_ROI_B3_UCL <- apply(PVA_ROI_B3, 1, quantile, probs = 0.9)
PVA_ROI_B3_means <- rowMeans(PVA_ROI_B3)

PVA_ROI_B3_df <- as.data.frame(cbind(
  years,
  (PVA_ROI_B3_means/2),
  (PVA_ROI_B3_LCL/2),
  (PVA_ROI_B3_UCL/2)
...
names(PVA_ROI_B3_df) <- c("years", "ROI_B3_mean", "ROI_B3_LCL", "ROI_B3_UCL")
write.csv(PVA_ROI_B3_df,
          "data/PVA_ROI_B3_means.csv",
          sep = ",",
          row.names = FALSE)
...
PVA_ROI_B4_LCL <- apply(PVA_ROI_B4, 1, quantile, probs = 0.1)
PVA_ROI_B4_UCL <- apply(PVA_ROI_B4, 1, quantile, probs = 0.9)
PVA_ROI_B4_means <- rowMeans(PVA_ROI_B4)

PVA_ROI_B4_df <- as.data.frame(cbind(
  years,
  (PVA_ROI_B4_means/2),
  (PVA_ROI_B4_LCL/2),
  (PVA_ROI_B4_UCL/2)
...
names(PVA_ROI_B4_df) <- c("years", "ROI_B4_mean", "ROI_B4_LCL", "ROI_B4_UCL")
write.csv(PVA_ROI_B4_df,
          "data/PVA_ROI_B4_means.csv",
          sep = ",",
          row.names = FALSE)
...
NI_B_means <- as.tibble(cbind(
  rowMeans(PVA_NI_B1[, -1])/2,
  rowMeans(PVA_NI_B2[, -1])/2,
  rowMeans(PVA_NI_B3[, -1])/2,
  rowMeans(PVA_NI_B4[, -1])/2
))

NI_B_means$years <- years
names(NI_B_means) <- c("B1", "B2", "B3", "B4", "years")
...
NI_B <- gather(NI_B_means,
               scenario, pop_size, gathercols)

write.table(NI_B,
            "data/HS_NI_B_means_simple_1.csv",
            sep = ",",
            row.names = FALSE)

NI_B_plot <- ggplot(NI_B) +
...
names(PVA_ROI_B_df) <- c("years", "mean", "lcl", "ucl")
names(PVA_ROI_B2_df) <- c("years", "mean", "lcl", "ucl")
names(PVA_ROI_B3_df) <- c("years", "mean", "lcl", "ucl")
names(PVA_ROI_B4_df) <- c("years", "mean", "lcl", "ucl")
...
RoI_B_pva$scenario <- rep(c("B1", "B2", "B3", "B4"), each = 31)
...
write.table(ROI_B,
            "data/HS_ROI_B_simple_1.csv",
            sep = ",",
            row.names = FALSE)

write.table(RoI_B_pva_2,
            "data/HS_ROI_B_simple_2.csv",
            sep = ",",
            row.names = FALSE)

ROI_B_plot <- ggplot(RoI_B_pva_2) +
  geom_line(aes(x = years,
                y = mean,
                group = scenario,
                colour = scenario),
