...
glen.K          <- 1000                  # carrying capacity of Glenwherry
...
write.table(simul.in,
            "data/curlew_HS_siteRem_20pairs_lplr_simple_params.csv",
            sep = ",",
            row.names = FALSE)
...
PVA_glen_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_lle_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_12_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
PVA_20_rem <- as.data.frame(cbind(
  years,
  PVA_min,
  PVA_med,
  PVA_max
...
glen_rem_pva <- as.tibble(rbind(
  PVA_glen,
  PVA_glen_rem
))

glen_rem_pva_means <- as.tibble(cbind(
  rowMeans(PVA_glen[, -1])/2,
  rowMeans(PVA_glen_rem[, -1])/2
))

glen_rem_pva_means$years <- years
names(glen_rem_pva_means) <- c("base", "removed", "years")



lle_rem_pva <- as.tibble(rbind(
  PVA_lle,
  PVA_lle_rem
))

lle_rem_pva_means <- as.tibble(cbind(
  rowMeans(PVA_lle[, -1])/2,
  rowMeans(PVA_lle_rem[, -1])/2
))

lle_rem_pva_means$years <- years
names(lle_rem_pva_means) <- c("base", "removed", "years")




pva_2_rem_means <- as.tibble(cbind(
  rowMeans(PVA_2[, -1])/2,
  rowMeans(PVA_2_rem[, -1])/2
))

pva_4_rem_means <- as.tibble(cbind(
  rowMeans(PVA_4[, -1])/2,
  rowMeans(PVA_4_rem[, -1])/2
))

pva_8_rem_means <- as.tibble(cbind(
  rowMeans(PVA_8[, -1])/2,
  rowMeans(PVA_8_rem[, -1])/2
))

pva_12_rem_means <- as.tibble(cbind(
  rowMeans(PVA_12[, -1])/2,
  rowMeans(PVA_12_rem[, -1])/2
))

pva_20_rem_means <- as.tibble(cbind(
  rowMeans(PVA_20[, -1])/2,
  rowMeans(PVA_20_rem[, -1])/2
))
...
gathercols <- c("base", "removed")

glen_rem <- gather(glen_rem_pva_means,
               scenario, pop_size, gathercols)


write.table(glen_rem,
            "data/curlew_glen_base_rem_1.csv",
            sep = ",",
            row.names = FALSE)

lle_rem <- gather(lle_rem_pva_means,
                  scenario, pop_size, gathercols)


write.table(lle_rem,
            "data/curlew_lle_base_rem_1.csv",
            sep = ",",
            row.names = FALSE)


pva_2_rem_means$years <- years
names(pva_2_rem_means) <- c("base", "removed", "years")

pva_4_rem_means$years <- years
names(pva_4_rem_means) <- c("base", "removed", "years")

pva_2_rem_means$years <- years
names(pva_2_rem_means) <- c("base", "removed", "years")

pva_8_rem_means$years <- years
names(pva_8_rem_means) <- c("base", "removed", "years")

pva_12_rem_means$years <- years
names(pva_12_rem_means) <- c("base", "removed", "years")

pva_20_rem_means$years <- years
names(pva_20_rem_means) <- c("base", "removed", "years")

rem_2 <- gather(pva_2_rem_means,
                   scenario, pop_size, gathercols)

rem_4 <- gather(pva_4_rem_means,
                scenario, pop_size, gathercols)
rem_8 <- gather(pva_8_rem_means,
                scenario, pop_size, gathercols)
rem_12 <- gather(pva_12_rem_means,
                scenario, pop_size, gathercols)
rem_20 <- gather(pva_20_rem_means,
                scenario, pop_size, gathercols)



# Plot
glen_rem_plot <- ggplot(glen_rem) +
