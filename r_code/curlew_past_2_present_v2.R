...
PVA <- function(
  ...
  # Create list to store each rep values
  Pop.Rep <- list()
  ...
  for (rep in 1:n.reps) {
    ...
    pop.vals <- data.frame(matrix(nrow = 0,
                                  ncol = 8)
                           )
    colnames(pop.vals) <- c("Initial Pop",
                            "Num Adults",
                            "Num Chicks",
                            "Num HS",
                            "Year Productivity Rate",
                            "Num Fledge Return",
                            "Year Catastrophe",
                            "Year Fail"
                            )
    
    # Fill simulation vlaues
    pop.vals[1, "Initial Pop"]  <- (init.N * 2)
    pop.vals[1, "Num Adults"]   <- (init.N * 2)
    pop.vals[1, "Num Chicks"]   <- 0
    pop.vals[1, "Num HS"]       <- 0
    pop.vals[1, "Year Productivity Rate"] <- S.jw.loop
    pop.vals[1, "Num Fledge Return"]  <- 0
    pop.vals[1, "Year Catastrophe"]   <- 0
    pop.vals[1, "Year Fail"]          <- 0
    ...
    for (y in 2:(n.years + 1)) {
      ...
      # If the breeding year is a failure, fecundity for the year is halved
      ...
      #pop.size  <- c((init.N/3), (init.N/3)*2) # this is the vector for
                                                # the starting populations
                                                # as our counts are adults
                                                # only, maybe this should be
                                                # a 0, 100% split?
      
      projections <- pop.projection(a,
                                    #les.mat
                                    n = pop.size,
                                    iterations = 30) # 30 years
      ...
      # Fill values
      pop.vals[y, "Initial Pop"]  <- (next.year)
      pop.vals[y, "Num Adults"]   <- pop.size[2]
      pop.vals[y, "Num Chicks"]   <- pop.size[1]
      pop.vals[y, "Num HS"]       <- 0
      pop.vals[y, "Year Productivity Rate"] <- S.jw.loop
      pop.vals[y, "Num Fledge Return"]  <- 0  # not in yet
      pop.vals[y, "Year Catastrophe"]   <- 0  # need object for catast.
      pop.vals[y, "Year Fail"]          <- 0  # need to change F.year part 
                                              # so we have an object for rbinom
      
    }
    
    Pop.Rep[[rep]] <- pop.vals
