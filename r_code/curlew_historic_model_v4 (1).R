...
saveRDS(ROI.sim.out,
        "data/ROI_historic_4stage_hphr_sims_1.RDS")
...
write.table(ROI.sim.out.df.long,
            "data/ROI_historic_means_hphr_fullsims_2.csv",
            sep = ",",
            row.names = FALSE)

ROI.pop.ests <- data.frame(years = c(1987, 2001, 2017),
                          popsize = c((2*7000), (2*1000), (2*150)))
...
write.table(ROI.sim.out.df.match,
            "data/ROI_historic_hphr_means_match_2.csv",
            sep = ",",
            row.names = FALSE)
...
ROI.sim.out.plot.match.ribbon <- ROI.sim.out.plot.match + 
  geom_ribbon(aes(x = years,
                  ymin = lcl,
                  ymax = ucl),
              alpha = 0.3) + 
