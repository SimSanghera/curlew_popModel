######  Head-Starting Curlew Population Viability Analysis Glenwherry #####
...
write.table(simul.in,
            "data/NI_HS_scenario_A1_base_hphr_parameters.csv",
            sep = ",",
            row.names = FALSE)
...
# Write to save file
saveRDS(NI.sim.out,
        "data/NI_HS_base_A1_sims_mphr_1.RDS")
...
write.table(NI.sim.out.df.long,
            "data/NI_HS_base_A1_means_mphr_fullsims_1.csv",
            sep = ",",
            row.names = FALSE)
...
# Lower than x
bettermatch.200 <- which(NI.sim.out.200[31 ,] < 5,
                         arr.ind = TRUE)
...
bettermatch.225 <- which(NI.sim.out.225[31 ,] < 5,
                         arr.ind = TRUE)
...
bettermatch.250 <- which(NI.sim.out.250[31 ,] < 5,
                         arr.ind = TRUE)
...
bettermatch.275 <- which(NI.sim.out.275[31 ,] < 5,
                         arr.ind = TRUE)
...
bettermatch.300 <- which(NI.sim.out.300[31 ,] < 5,
                         arr.ind = TRUE)
...
write.table(NI.sim.out.df.match,
            "data/NI_HS_base_A1_mphr_means_match5_1.csv",
            sep = ",",
            row.names = FALSE)
