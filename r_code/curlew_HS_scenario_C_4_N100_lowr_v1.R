...
# Head-Starting Scenario C - courtesy of Becs Lee
# Underlying productivity does not change, low productivity continues
#   in the long-term
#   1. Scenario C4: Change release to every 2 years for 5 installments
#       & 50% relay - which means reduce productivity by 50% for each
#       individual, birds don't return
#       The model will be split into 4 different models:
...
hs.pop        <- 25                     # number of clutches

capt.fail     <- 0.2                    # The number of HS birds that
# fail during rearing & can't be
# released

intro.years   <- c(1, 3, 5, 7, 9)       # years in which hs birds will be
# introduced
...
bird.matrix.2 <- expression(
  ...
  0,          0,     0,                       0,                      0,
  S.jw*phil.p, (S.jhs*capt.fail)*(phil.p*0),  0,  0,                      0,
  0,          0,      S.imm*return.p,         0,                      0,
  0,          0,      0,                      S.sub*return.p,     S.ad*return.p
)
